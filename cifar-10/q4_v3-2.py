...
model = Sequential()
model.add(Convolution2D(32, 3, 3, border_mode='same', input_shape=(img_channels, img_rows, img_cols), init='he_normal'))
...
model.add(Dense(256, init='he_normal', W_regularizer=l2(0.001), b_regularizer=l2(0.001)))
model.add(Activation('relu'))
model.add(BatchNormalization())
model.add(Dropout(0.5))
...
pickle.dump( history.history, open( "q4_v3-2.p", "wb" ) )
